---
name: session-reader
description: 在需要读取用户指定的 Codex、Claude、Gemini 或导出会话的指定范围时使用；只读输出标准化会话包，不复制原始记录或写入正式记忆。
---
...
- 指定厂商会话：`node scripts/session-reader.js read --provider <codex|claude|gemini> --session <会话ID>`。
