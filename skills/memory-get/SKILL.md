...
按 `../../rules/engineering-principles.md` 执行；先读 `references/framework-and-retrieval.md`。只读，不写正式 `docs/`。

- 先检查双向索引和策略。冲突、未选择的旧记忆或项目外目标停止处理。
...
- 输出约束、入口、历史意图、未知项、检索回执(receipt)或跳过原因，并列全部实际“记忆引用”。
